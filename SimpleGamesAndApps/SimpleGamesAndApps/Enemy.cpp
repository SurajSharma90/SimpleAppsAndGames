...
const sf::Vector2f Enemy::getNormVec(sf::Vector2f fromPos) const
{
  sf::Vector2f dirVec;
  sf::Vector2f normVec;

  dirVec = this->getCenter() - fromPos;

  float magnitude = static_cast<float>(sqrt(pow((double)dirVec.x, 2)+pow((double)dirVec.y, 2)));

  normVec = dirVec/magnitude;
